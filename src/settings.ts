...
export class PDFPlusSettingTab extends PluginSettingTab {
    ...
    async display(): Promise<void> {
        ...
        this.addSliderSetting('dummyPdfCacheSize', 0, 100, 1)
            .setName('Dummy PDF cache size')
            .setDesc('To improve performance and reduce network bandwidth, downloaded dummy PDFs are cached in the browser\'s local IndexedDB. If the number of cached files exceeds this limit, old files will be deleted.');

        this.addSetting()
            .setName('Clear dummy PDF cache')
            .setDesc('Clear the cache of downloaded dummy PDFs.')
            .addButton((button) => {
                button.setButtonText('Clear').onClick(async () => {
                    await this.plugin.lib.clearDummyPdfCache();
